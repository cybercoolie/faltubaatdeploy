...
echo "[INFO] FaltuBaat - App Server Installation"
echo "======================================="
...
echo ""
echo "[INFO] Will download code from:"
...
if [ -z "$RTMP_SERVER_IP" ]; then
    echo "[WARN] No RTMP server IP provided. Streams won't work until configured."
...
if [ -d "$APP_DIR" ]; then
    echo "[INFO] Cleaning up existing app directory..."
    sudo rm -rf $APP_DIR
fi
...
echo "[INFO] Downloading application code from GitHub..."
TEMP_DIR=$(mktemp -d)
...
echo "[INFO] Updating npm to latest version..."
...
echo ""
echo "[OK] App Server Installation complete!"
echo "======================================="
echo ""
...
echo ""
echo "[INFO] IMPORTANT - Configure RTMP Server:"
