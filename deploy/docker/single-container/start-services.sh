...
echo "🚀 Starting FaltuBaat Single-Container (Node.js + Nginx/RTMP)..."
...
if [ "$S3_SYNC_ENABLED" = "true" ] && [ -n "$S3_DB_BUCKET" ]; then
    ...
    echo "📦 Using local/EFS storage for database"
fi
...
if pgrep nginx > /dev/null; then
    echo "✅ Nginx RTMP server started on ports 1935 (RTMP) and 8080 (HLS)"
else
